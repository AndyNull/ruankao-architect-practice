...
- 模式切换会重置跨科筛选条件；继续练习和记忆复习同时显示总题数与本次题数。
- 真题套卷在右侧答题卡提供当前年份/批次选择器，并按考试期正确排序。
- 优化桌面端和移动端科目切换、工具栏与提示显示。
- 使用 GitHub Actions 自动部署 GitHub Pages。完整记录见 [CHANGELOG.md](CHANGELOG.md)。
- 侧栏在线人数暂未接入统计服务；纯 GitHub Pages 无法获取真实在线会话，不显示伪造数字。
- 侧栏“陪学估算”仅按本地时间桶生成约数，不连接服务、不采集或保存在线信息。
...
当前五科选择题均无待补答案：架构师 1947 道、系规 814 道、高项 3686 道、系分 956 道、网规 1496 道。
...
系统架构设计师选择题中真题 1422 道、模拟题 525 道。真题选择题覆盖 `2009年下半年` 到 `2025年下半年` 的可结构化公开资料；其中 `2010`、`2011` 年各缺第 8 题，`2012` 年缺第 69 题。案例与论文只保留题干和答案均可校验的条目；架构师 `2024年上半年` 案例第 4 题原始资料只有考点摘要，已明确标记缺口而不以占位内容展示。题面和答案均为第三方整理资料，不代表官方原卷或官方答案。
